    ...
    def combine(self, good_chromosomes, strict=False):
        ...
        if not good_chromosomes:
            ...
            try:
                ...
                mitoRate = 0
            """
            header = ["Total_read", "Distinct_read",
                      "Single-Mapped", "Multi-Mapped", "Duplicate_rate",
                      "NRF", "PBC1", "PBC2"]
            np.savetxt(self.outfile, np.c_[total, stats['M_DISTINCT'],
                       stats['M1'], M2, dupRate, NRF, PBC1, PBC2],
                       header='\t'.join(header), fmt='%s', delimiter='\t',
                       comments='''NRF=non redundant fraction(Distinct_read/Total_read)\nPBC1=PCR Bottleneck coefficient 1(Single-Mapped/Distinct_read)\nPBC2=PCR Bottleneck coefficient 2(Single-Mapped/Multi-Mapped)\nPBC1 measure:\n 1. 0-0.5 is severe bottlenecking\n 2. 0.5-0.8 is moderate bottlenecking\n 3. 0.8-0.9 is mild bottlenecking\n 4. 0.9-1.0 is no bottlenecking\n''')
